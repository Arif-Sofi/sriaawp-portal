...
- NEXT: PR4 `feat/takwim` (Takwim/calendar: event, event_occurrence, event_audience, room, blackout_window tables; public cached feed + role calendar views), branched off `feat/news`.
...
- Migration numbering: drizzle-kit numbers by journal idx, but the hand-authored RLS files (`0001_rls_policies`, `0003_content_rls`) sit outside drizzle's journal. After `npm run db:generate`, rename the generated `000N_*.sql` to the next FREE sequential number, rename its `meta/000N_snapshot.json`, and update the tag in `meta/_journal.json`; then `npm run db:check` must say "Everything's fine". Apply only the NEW files to Supabase via `node -r dotenv/config scripts/_apply.mjs <files> dotenv_config_path=.env.local` (0000/0001 lack IF NOT EXISTS, so never re-run them).
