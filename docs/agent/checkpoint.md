...
- NEXT: PR5 `feat/events-conflict` (event creation form + conflict checker per master-plan 11.6: room/exam HARD, organizer/blackout SOFT, RRULE occurrence expansion, admin override flow), branched off `feat/takwim`.
...
- Calendar (PR4): `@/lib/calendar/queries` (listPublicOccurrences, listVisibleOccurrences({fromISO,toISO,user}), listBlackouts). Tables: room, event (status draft|published|pending_review|cancelled, priority normal|exam, rrule), event_occurrence (materialized; one per event so far), event_audience (audience_type public|role|department + audience_ref), blackout_window (scope school|department, is_hard). Visibility: a published event with a public audience is shown to anyone; role/department audiences only to matching users. Live: `/takwim`, cached `/api/takwim`. Reuse `@/components/ui/calendar`. Display dates in Asia/Kuala_Lumpur.
