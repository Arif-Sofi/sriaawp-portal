...
| #98 | `feat/ai-mode-3` | `feat/ai-modes-1-2` | #81 | none | In progress. Mode-3 manual RAG: re-ingestable manual pipeline + flat-scan retrieval + tau_refuse refusal + Supabase Storage split (resolves #89). Real manual pending PM (#88). Live runs gated on Gemini key. |
