...
| #101 | `spike/facebook-graph-api` | `feat/engagement-ui` | #84 | none | Open. FB feasibility write-up + env-selected FacebookClient interface + MockFacebookClient (no real Meta) + R-13..R-16 re-rate. |
| #102 | `feat/facebook-outbound` | `spike/facebook-graph-api` | #85 | 0010 | In progress. Outbound MVP vs mock: fb_sync_link + news.origin/external_id + encrypted fb_credential; public-only allow-list guard (both boundaries); content_hash dedup; outbox worker + job route; admin toggle + kill-switch. |
