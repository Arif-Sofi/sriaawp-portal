...
export function LoginForm({ action, googleAction, errorCode }: LoginFormProps) {
  return (
...
function GoogleButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-200 disabled:cursor-not-allowed disabled:opacity-60"
    >
      {pending ? "Mengalih… / Redirecting…" : "Log masuk dengan Google / Sign in with Google"}
    </button>
  );
}
